...
class QNet(nn.Module):
    ...
    def __init__(self, num_channel, is_occ, num_actions, is_goal, drop_rate, layers=[2, 2, 2, 2]):
        ...
        self.conv1 = nn.Conv2d(
            num_channel + int(self.conf) + self.is_goal, 
            self.inplanes, kernel_size=7, stride=2, padding=3
        )
        self.relu = nn.LeakyReLU(inplace=True)
        ...
        self.drop_2 = nn.Dropout(p=self.drop_rate) # MC Dropout 
        self.layer3 = self._make_layer(block, 256, layers[2], stride=1)
        self.drop_3 = nn.Dropout(p=self.drop_rate) # MC Dropout 
